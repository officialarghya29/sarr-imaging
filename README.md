# <div align="center">◆ SAR-YOLO / SARVO</div>

<div align="center">

**Speckle-aware, acquisition-conditioned object detection for synthetic aperture radar**
...
| **The paper** | Written from evidence, not around it: section skeletons in [`reports/paper/`](reports/paper/), hostile review in [`reports/red_team_review.md`](reports/red_team_review.md). |
| **Second architecture** | RT-DETR feasibility arm built: the adapter inserts into an RT-DETR graph via `SARYOLORTDetectionModel` (variant generated by `scripts/make_rtdetr_variant.py`). Feasibility only — no cross-architecture number exists yet. |
...
## Part X · Paper assembly (SARVO Phase 55)

The paper is *assembled from measured evidence*, not written around a hoped-for result. Every
section skeleton lives in [`reports/paper/`](reports/paper/) and carries its evidence source
and its blocked status:

```text
reports/paper/
├── README.md          section index: what feeds what, and the four writing rules
├── abstract.md        BLOCKED-ON-RUN — written last, measured results only
├── introduction.md    narrative arc + candidate contributions (dropped if unsupported)
├── related_work.md    five literatures; defers to docs/research_gap.md
├── method.md          subsection ↔ code map (no component described that does not exist)
├── experiments.md     protocol/controls already fixed; hardware+numbers blocked
├── ablations.md       table plan: which claim each generated table can/cannot carry
└── limitations.md     ordered by the red-team review's severity ranking
```

Companion artefacts already in the repository:

* `paper/manuscript/main.tex` — CVPR-style skeleton; every table input is generated.
* `paper/tables/` — `python -m saryolo assets` output; `TBD` = not measured, never filled.
* `reports/red_team_review.md` — the hostile review; measurement-dependent verdicts are
  marked `BLOCKED-ON-RUN` and must be re-answered before submission.
* `docs/PROGRESS.md` — the living map of what is **built** vs what is **measured**.

The submission gate is mechanical: `python -m saryolo assets --require-complete` fails while
any table cell is unmeasured — the paper cannot be assembled around missing evidence even by
accident.

## Part XI · Reproducibility
